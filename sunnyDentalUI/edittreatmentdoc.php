...
<style>
body{background:#142530;margin:0}
.form{width:340px;height:620px;background:#e6e6e6;border-radius:8px;box-shadow:0 0 40px -10px #000;margin:calc(50vh - 220px) auto;padding:20px 30px;max-width:calc(100vw - 40px);box-sizing:border-box;font-family:'Montserrat',sans-serif;position:relative}
...
<body translate="no">
  ...
  <form id="tretformdoc" class="form" action="edittreatsql.php" method="POST">
  <?php
	// database connection 
	include("dbconn.php");
	
	// display your profile immediately after login
	$sql = "SELECT * FROM patient pat INNER JOIN appointment AS a ON a.patientIC = pat.patientIC INNER JOIN treatment t ON t.treatmentID = a.treatmentID INNER JOIN employee e on e.empID = a.empID WHERE appID = ".$_GET['snoupdapp'];
  ...
  <p type="Previous treatment inserted:"><input type="text" name="treatname" value="<?php echo $r['treatmentName']?>"  readonly/></p>
  <p type="Insert Treatment:"><label type="text" name="treatmentold" placeholder="What treatment have you do?"></label>
